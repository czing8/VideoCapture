...
@implementation VScaleButton {
    CALayer *_effectiveLayer;
    ScaleButtonState _state;
...
- (void)panAction:(UILongPressGestureRecognizer *)ges {
    ...
    switch (ges.state) {
        ...
        case UIGestureRecognizerStateBegan: {
            _state = ScaleButtonStateIn;
            if (self.stateChange) {
                self.stateChange(ScaleButtonStateBegin);
            }
        ...
        case UIGestureRecognizerStateChanged: {
            ...
            if (![self circleContainsPoint:point]) {
                NSLog(@"out");
                _state = ScaleButtonStateOut;
                if (self.stateChange) self.stateChange(ScaleButtonStateOut);
                ...
                NSLog(@"in");
                if (self.stateChange)  self.stateChange(ScaleButtonStateIn);
                _state = ScaleButtonStateIn;
            }
            break;
        }
        ...
        case UIGestureRecognizerStateCancelled: {
            NSLog(@"cancel, end");
            if(_state == ScaleButtonStateIn){
                 if (self.stateChange) self.stateChange(ScaleButtonStateFinish);
            }else{
                 if (self.stateChange) self.stateChange(ScaleButtonStateCancle);
            }
            break;
        }
